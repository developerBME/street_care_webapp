...
>>>>>>> bce5802c541eae32ce8c21faa4a8c015aa5b0963
		return {
				username: '',
				photoUrl: '',
		};
	...
	try {
		...
		for (const doc of snapshot.docs) {
			...
			if (
				...
				eventData.participants.includes(uid)
			) {
				const result = await fetchUserDetails(eventData.uid);
				const userName = result.username;
				const photoUrl = result.photoUrl;
			...
			if (user) {
				try {
					...
					if (currentParticipants.includes(fAuth.currentUser.uid)) {
						...
						console.log(
							'successfully added to user to outreach collection'
						);
					}
					...
					if (currentEvents.includes(id)) {
						...
						const newEvents = [...currentEvents, id];
						const userDocUpdate = doc(
							db,
							USERS_COLLECTION,
							userDocID
						);
						if (isBMEFlow) {
							const userUpdateRef = await updateDoc(
								userDocUpdate,
								{
									officialEvents: newEvents,
								}
						...
						console.log(
							'successfully added outreach to users collection'
						);
					}
					setLabel2('EDIT');
				} catch (error) {
				...
				try {
					...
					if (currentParticipants.includes(fAuth.currentUser.uid)) {
						...
						const eventDocUpdate = isBMEFlow
							? doc(db, OFFICIAL_EVENTS_COLLECTION, id)
							: doc(db, OUTREACH_EVENTS_COLLECTION, id);
						const i = currentParticipants.indexOf(
							fAuth.currentUser.uid
						);
						if (i > -1) {
							currentParticipants.splice(i, 1);
					...
					logEvent(
						'STREET_CARE_ERROR',
						`error on rsvp edit- ${error.message}`
					);
					throw error;
				}
			} else {
	...
	try {
		...
		for (const doc of eventSnapshot.docs) {
			...
			userOutreaches.push({
				...
				id: doc.id,
				label:
					user && currentParticipants.includes(user.uid)
						? 'EDIT'
						: 'RSVP',
				nop: currentParticipants.length,
				photoUrl: photoUrl,
