...
const Eventcard = () => {
    return (
      <div className="event-card bg-green-100 rounded-lg p-4 md:p-8">
      <div class="flex items-center gap-2">
        <img class="w-9 h-9 md:w-9 md:h-9 rounded-full" src={userImg} alt="User Avatar" />
        <div class="text-black text-xs md:text-sm font-normal">William Smith</div>
        <div class="bg-emerald-100 rounded-xl border border-stone-300 flex items-center gap-1">
            <img class="w-4 h-4" src={verifiedImg} alt="Verified" />
            <div class="text-zinc-700 text-xs md:text-sm font-semibold leading-tight">Certified Lead</div>
        </div>
        </div>
        <div className="mt-2 md:mt-4">
          <div className="text-neutral-800 text-sm md:text-lg font-medium leading-loose">
            BK Fort Green Outreach
          </div>
          <div className="text-zinc-700 text-xs md:text-sm font-semibold leading-normal">
            Sept 9, 2023 SAT 12:00pm
          </div>
          <div className="mt-2 md:mt-4 p-2 bg-white rounded-lg flex items-center gap-2.5">
            <img src={wavingHand} alt="Waving Hand" />
            <div className="text-neutral-900 text-xs md:text-sm font-normal leading-snug">
              Childcare specialist needed
            </div>
          </div>
          <div className="mt-2 md:mt-4 flex flex-col gap-1">
            <div className="text-zinc-700 text-sm md:text-lg font-normal">
        ...
        <div className="mt-2 md:mt-4">
          <div className="pt-3 flex items-center gap-2.5">
            <button className="bg-violet-950 text-white py-2 px-4 rounded-full text-xs md:text-sm">
              Join
            </button>
          </div>
        </div>
