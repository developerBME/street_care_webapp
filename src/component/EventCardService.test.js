...
describe("calculateNumberOfPages function", () => {
  test("calculates the number of pages correctly", async () => {
    ...
    expect(getDocs).toHaveBeenCalledWith(collection(db, OUTREACH_EVENTS_COLLECTION));
  });
...
describe("fetchByCityOrState", () => {
  ...
  test("returns outreach data for a given city and date range", async () => {
    ...
    expect(collection).toHaveBeenCalledWith(db, PAST_OUTREACH_EVENTS_COLLECTION);
...
describe('fetchVisitLogsByCityOrState function', () => {
  ...
  test("returns empty array if no documents found", async () => {
    ...
    expect(getDocs).toHaveBeenCalledWith(query(collection(db, PERSONAL_VISITLOG_COLLECTION), 
