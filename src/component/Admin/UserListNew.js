...
export default function UserListNew() {
 ...
 useEffect(() => {
   ...
   fetchData();
 }, [cursorFields.direction, currentPage, filter, searchTerm]);
 ...
 const changeUserType = async (email, docId, Type) => {
   try {
     if (!email || !docId || !Type) {
       throw new Error(
         "Invalid input: All parameters (email, docId, Type) are required."
       );
     }
     ...
     if (!validUserTypes.includes(Type)) {
       throw new Error(
         `Invalid Type: "${Type}" is not a recognized user type.`
       );
     }
     ...
     console.log(
       `User type updated successfully to "${Type}" for email: ${email}`
     );
   } catch (error) {
