...
const ApprovalCardVisitlogs = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start">
        <div className="flex flex-col space-y-2">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-[#37168B] font-medium">{formattedDate}</span>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-[#37168B] font-medium">
              {postData?.location?.city || postData?.city
                ? `${postData.location?.city || postData.city}${
                    postData.stateAbbv || postData.state ? `, ${postData.stateAbbv || postData.state}` : ""
                  }`
                : "Unknown City"}
            </span>
          </div>
      ...
      {/* Middle Section: Title & Description with Scroll */}
      <div className="mt-4">
        <h1 className="text-lg font-medium text-[#1F0A58] line-clamp-1">
          {postData.peopleHelpedDescription || "Event Title"}
        </h1>
        <div className="text-sm text-[#444746] mt-2 max-h-[100px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent hover:scrollbar-thumb-gray-600">
          {postData.peopleHelpedDescription || "No description available."}
        </div>
      </div>

      {/* Tags Section */}
      <div className="mt-4 flex flex-wrap gap-2">{getTags(postData, isVisitLogs)}</div>
    </div>
  );
};
